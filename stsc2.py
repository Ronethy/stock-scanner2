...
def render_chart(df, symbol, timeframe, score_history, plan):
    fig = make_subplots(
        rows=4,
        cols=1,
        shared_xaxes=True,
    ...
    # Entry / SL / TP
    if plan:
        fig.add_hline(y=plan["Entry"], line_dash="dot", line_color="green", row=1, col=1)
        fig.add_hline(y=plan["Stop"], line_dash="dot", line_color="red", row=1, col=1)
        fig.add_hline(y=plan["Target"], line_dash="dot", line_color="blue", row=1, col=1)
...
with tab_early:
    ...
    early_df = scan_early_movers(SP500_SYMBOLS, client, 20)
    ...
    if early_df.empty:
        ...
        for _, row in early_df.iterrows():
            ...
            col1.write(f"**{row['Symbol']}** – Gap {row['Gap %']:.2f}%")
            if col2.button("⚡ Daytrade", key=f"d_{row['Symbol']}"):
                st.session_state.selected_symbol = row["Symbol"]
            if col3.button("🧭 Swing", key=f"s_{row['Symbol']}"):
                st.session_state.selected_symbol = row["Symbol"]
...
with tab_sp500:
    ...
    for symbol in SP500_SYMBOLS:
        try:
            req = StockBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Day, limit=80)
            ...
            rows.append({"Symbol": symbol, "Score": score, "Ampel": traffic_light(score)})
        except Exception:
            continue
    ...
    for _, row in table.iterrows():
        ...
        col1.write(f"**{row['Symbol']}** | Score: {row['Score']} {row['Ampel']}")
        if col2.button("📈 Öffnen", key=f"open_{row['Symbol']}"):
            st.session_state.selected_symbol = row["Symbol"]
...
with tab_day:
    ...
    symbol = st.selectbox(
        "Aktie",
        SP500_SYMBOLS,
        index=SP500_SYMBOLS.index(st.session_state.selected_symbol)
    )
    ...
    if bars is not None and not bars.empty:
        ...
        for i in range(20, len(df)):
            s = MarketSnapshot(
                symbol, df.iloc[i]["close"], df.iloc[i]["rsi"],
                df.iloc[i]["ema9"], df.iloc[i]["ema20"], df.iloc[i]["ema50"],
                df.iloc[i]["atr"], 1.0, market_state
            )
            history.append(calculate_trend_score(s))

        render_chart(df, symbol, "1-Min", history, plan)
...
with tab_swing:
    ...
    req = StockBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Day, limit=200)
    bars = client.get_stock_bars(req).df
    if bars is not None and not bars.empty:
        df = bars.xs(symbol) if isinstance(bars.index, pd.MultiIndex) else bars.copy()
        df["ema9"] = ema(df["close"], 9)
        ...
        render_chart(df, symbol, "Daily", history, plan)
